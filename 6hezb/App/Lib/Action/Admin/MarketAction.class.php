...
class MarketAction extends ACommonAction
{
    public function index(){
		...
		$mlogs = M('member_integrallog l')->field("l.type,sum(l.affect_integral) AS integral")->join("{$this->pre}members m ON m.id=l.uid")->where($map)->group("l.type")->select();
		...
		$count = M('member_integrallog l')->join("{$this->pre}members m ON m.id=l.uid")->where($map)->count('l.id');
		...
		$order = "l.id DESC";
		$list = M('member_integrallog l')->field($field)->join("{$this->pre}members m ON m.id=l.uid")->where($map)->order($order)->limit($Lsql)->select();
	...
	public function getlog(){
		...
		//分页处理
		import("ORG.Util.Page");
		$count = M('market_log l')->join("{$this->pre}members m ON m.id=l.uid")->where($map)->count('l.id');
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理

		$mlogs = M('market_log l')->field("type,count(l.id) AS num,sum(l.price) AS money")->join("{$this->pre}members m ON m.id=l.uid")->where($map)->group("type")->select();
		foreach ($mlogs as $k => $v) {
			$ty_list[$v['type']] = $v;
		}
		...
		$money = M('market_log l')->join("{$this->pre}members m ON m.id=l.uid")->where($map)->sum("l.price");
		...
		$logway = C('MARKET_WAY');
		$list = M('market_log l')->field("l.*,m.user_name")->join("{$this->pre}members m ON m.id=l.uid")->where($map)->order("l.id DESC")->limit($Lsql)->select();
    ...
    public function doGoodEdit(){
		...
		/*if(!empty($_FILES['imgfile']['name'])){
			...
			$this->savePathNew = '/Style/market/images/'. date("YmdHis",time()).rand(0,1000)."/" ;
	...
	public function lottery(){
		...
		//分页处理
		import("ORG.Util.Page");
		$count = M('market_jifenlist')->count('id');
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";

		$list = M('market_jifenlist')->limit($Lsql)->select();
		foreach($list as $key=>$v){
			if($v['category']=='1'){
    ...
    public function lottery_edit(){
    	if($_REQUEST['id']){
			$id = intval($_REQUEST['id']);
			$good = M('market_jifenlist')->where("id={$id}")->find();
	        $this->assign("vo", $good);
		}
...
//      $data['add_time'] = time();
	...
	public function lottery_del(){
    	if(!empty($_REQUEST['delid'])){
			$id = intval($_REQUEST['delid']);
			$del = M('market_jifenlist')->where("id={$id}")->delete();
		}

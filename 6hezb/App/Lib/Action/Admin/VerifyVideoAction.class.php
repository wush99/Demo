<?php
// 全局设置
class VerifyVideoAction extends ACommonAction
{
    /**
    +----------------------------------------------------------
    * 默认操作
    ...
    */
    public function index()
    {
		...
		$count = M('video_apply v')->join("{$this->pre}members m ON m.id=v.uid")->join("{$this->pre}member_money mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->where($map)->count('m.id');
		...
		$list = M('video_apply v')->field($field)->join("{$this->pre}members m ON m.id=v.uid")->join("{$this->pre}member_money mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->where($map)->limit($Lsql)->order('v.id DESC')->select();

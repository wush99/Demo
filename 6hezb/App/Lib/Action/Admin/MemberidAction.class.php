...
class MemberidAction extends ACommonAction
{
    ...
    */
	...
	public function export(){
		...
		//$page = $p->show();
		//$Lsql = "{$p->firstRow},{$p->listRows}";
		$list = M('members_status s')->field('s.uid,s.id_status,mi.up_time,mi.card_img,mi.card_back_img,mi.real_name,mi.idcard,m.user_name')->join("{$pre}members m ON m.id=s.uid")->join("{$pre}member_info mi ON mi.uid=s.uid")->where($map)->order("mi.up_time DESC")->select();//->limit($Lsql)

...
class ToolAction extends HCommonAction {
    ...
    public function index(){
		if($_POST){
			...
			$repay_detail['borrow_manage'] = round($amount*$borrow_manage*$date_limit/100,2);//基金管理费
				...
				case '3'://每月还息到期还本
					...
					$repay_detail['day_apr'] = round($repay_detail['month_apr']*12/365,2);
  					...
  					for($i=0;$i<$date_limit;$i++){
	  					if ($i+1 == $date_limit)	$capital = $amount;//本金只在最后一个月还，本金等于基金金额除季度
					  	else 	$capital = 0;
		...
		$this->assign("leftlist",array(1=>array("turl"=>"/tools/tool2.html","type_name"=>"投资计算器"),0=>array("turl"=>"/tools/tool.html","type_name"=>"项目计算器")));
		$this->display();
	}
    ...
    public function tool2(){
		if($_POST){
			// var_dump($_POST);exit();
			$amount = round(floatval($_POST['amount']),2);//募集金额
			$date_limit = intval($_POST['date_limit']);//投资期限
			$rate = floatval($_POST['rate']);//投资利率
			$reward_rate = floatval($_POST['reward_rate']);//基金奖励
			...
			switch ($repayment_type) {
				...
				case '3'://每月还息到期还本
					...
					$repay = $repay_detail['repayment_money'];
  					for($i=0;$i<$date_limit;$i++){
	  					if ($i+1 == $date_limit){
	  						$capital = $amount;//本金只在最后一个月还，本金等于基金金额除季度

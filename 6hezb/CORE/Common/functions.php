...
function U($url,$vars='',$suffix=true,$redirect=false,$domain=false) {
	...
	if($asdd){
		foreach (C('APP_SUB_DOMAIN_RULES') as $keyr => $ruler) {
			if(0=== stripos($url,$ruler[0]))	$isDomainGroup = true;
		}
	}
	//#########及默认分组不需要加分组名
	if(GROUP_NAME == C('DEFAULT_GROUP')) $isDomainGroup = true;
    ...
    if($domain===true){
        ...
        if($asdd) { // 开启子域名部署
            ...
            foreach (C('APP_SUB_DOMAIN_RULES') as $key => $rule) {
                ...
                if(false === strpos($key,'*') && 0=== stripos($url,$rule[0])) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由  //****前面带/的***如果给U参数传入的地址第一位是“/”,就会走这条路,如果设置了带域名即$domain=true，那么url在传到这里之前就会被处理，不带/
            $route   =  true;
            $url   =  substr($url,1);//****前面带/的****然后在这里把第一位的"/"删掉
            if('/' != $depr) {
                $url   =  str_replace('/',$depr,$url);
            }
        }else{//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^会走这条路
            if('/' != $depr) { // 安全替换 参数的分隔方式，一般不会变
                $url   =  str_replace('/',$depr,$url);
            }
            // 解析分组、模块和操作
            $url   =  trim($url,$depr);//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^去掉头尾的分隔符，这里是“/”
			
            $path = explode($depr,$url);
            $var  =  array();
            $var[C('VAR_ACTION')] = !empty($path)?array_pop($path):ACTION_NAME;//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^返回$path的最后一个元素并删除
            $var[C('VAR_MODULE')] = !empty($path)?array_pop($path):MODULE_NAME;//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^返回$path的最后一个元素并删除
            if(C('URL_CASE_INSENSITIVE')) {
                $var[C('VAR_MODULE')] =  parse_name($var[C('VAR_MODULE')]);
            }
            if(C('APP_GROUP_LIST')) {//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^如果分组了，并且$path参数在删除上面两个元素后还不为空，则给var的gropu赋值$path的最后一个元素值
                if(!empty($path)) {
                    $group   =  array_pop($path);
                    if(!$isDomainGroup) $var[C('VAR_GROUP')]  =   $group;//#########修复##########，增加对当前分组分配了二级域名的判断,如果分组分配了二级域名，就不要体现在路径上了
                }else{
                    if(GROUP_NAME != C('DEFAULT_GROUP')) {
                        if(!$isDomainGroup) $var[C('VAR_GROUP')]  =   strtolower(GROUP_NAME);//####修复####，增加对当前分组分配了二级域名的判断,如果分组分配了二级域名，就不要体现在路径上了
                    }
                }
            }
        }
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(isset($route)) {//****前面带/的******接着在这里组成带根目录路径的url,是按传入的直接生成，没做什么处理
            $url   =  __APP__.'/'.$url;
        }else{ 
            $url   =  __APP__.'/'.implode($depr,array_reverse($var));//^^^^^^^前面没带/的^^^^^^^^^在这里组合上面的处理过的参数，生成带根目录的url，做了处理的　&&&&&&&&&&&&&&往下去和没带/的就没什么区别了
        }
        if(!empty($vars)) { // 添加参数
            $vars = http_build_query($vars);

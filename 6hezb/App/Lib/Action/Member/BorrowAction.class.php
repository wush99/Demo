...
class BorrowAction extends MCommonAction {
    ...
    public function borrow(){
		...
		else  $data['html'] = '<script type="text/javascript">alert("为保障资金安全，请填写募集人资料，通过募集人审核，再发标项目。点击确定后转到转到募集人审核页面。");window.location.href="'.__APP__.'/member/Memberinfo/index";</script>';
	...
	public function addborrow(){
		...
		if($vminfo['credit_status']==0) ajaxmsg("您还未通过项目审核",0);
		$bc = M("borrow_info")->where("borrow_uid={$this->uid} AND borrow_status in(0,2)")->count('id');
		if($bc>0) ajaxmsg("您有正在审核或者正在筹集中的项目，所以暂时不能发布新的项目申请",0);

    ...
    */
	...
	public function export(){
		...
		foreach($list as $key=>$v){
			...
			$interest_jiliang =M('borrow_investor')->where("borrow_uid={$uid}")->sum('reward_money');//累计支付项目奖励
			...
			$money['dxrtb'] = $investing[0]['capital'];//待确认项目
			...
			$money['ljjrje'] = $borrowANDpaid[0]['borrow_money'];//累计募集金额
		...
		//
			...
			$money['ljtbjl'] = $listarray['20']['money'];//累计项目奖励
		...
		$row=array();
		$row[0]=array('ID','用户名','真实姓名','总余额','可用余额','冻结金额','待收本息金额','待收本金金额','待收利息金额','待付本息金额','待付本金金额','待付利息金额','待确认项目','待审核提现+手续费','处理中提现+手续费','累计提现手续费','累计充值手续费','累计提现金额','累计充值金额','累计支付佣金','累计项目奖励','净赚利息','净付利息','管理员操作资金');
		$i=1;
		foreach($list as $v){
				$row[$i]['uid'] = $v['id'];

...
class MemberdataAction extends ACommonAction{
    public function index(){
		...
		$list = M('member_data_info mf')->field('mf.*,au.real_name,m.user_name as uname,m.customer_name')->join("{$this->pre}members m ON m.id=mf.uid")->join("{$this->pre}ausers au ON au.id=mf.deal_user")->where($map)->limit($Lsql)->order("mf.id DESC")->select();

    ...
    */
	...
	public function investcheck(){
		...
		if(!empty($binfo['password'])){
			if(empty($_POST['borrow_pass'])) ajaxmsg("此标是定向标，必须验证项目密码",3);
			else if($binfo['password']<>md5($_POST['borrow_pass'])) ajaxmsg("项目密码不正确",3);
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		if($binfo['money_collect']>0){
			if($vm['money_collect']<$binfo['money_collect']) {
				ajaxmsg("此标设置有项目待收金额限制，您账户里必须有足够的待收才能投此标",3);
			}
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		//项目总数检测
		...
		if(($capital+$money)>$binfo['borrow_max']&&$binfo['borrow_max']>0){
			...
			ajaxmsg("您已项目{$capital}元，此投上限为{$binfo['borrow_max']}元，你最多只能再投{$xtee}",3);
		}
		
		$need = $binfo['borrow_money'] - $binfo['has_borrow'];
		$caninvest = $need - $binfo['borrow_min'];
		if( $money>$caninvest && ($need-$money)<>0 ){
			$msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您项目{$money}元，将导致最后一次项目最多只能投".($need-$money)."元，小于最小项目金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>或者项目金额必须<font color='#FF0000'>小于等于{$caninvest}元</font>";
			if($caninvest<$binfo['borrow_min']) $msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您项目{$money}元，将导致最后一次项目最多只能投".($need-$money)."元，小于最小项目金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>即项目金额必须<font color='#FF0000'>等于{$need}元</font>";

			ajaxmsg($msg,3);
		}
		if(($binfo['borrow_min']-$money)>0 ){
			$this->error("尊敬的{$uname}，本标最低项目金额为{$binfo['borrow_min']}元，请重新输入项目金额");
		}
		if($money % $binfo['borrow_min']){
		    ajaxmsg("项目金额必须为最小投资的整数倍！",0);
	    }
		if(($need-$money)<0 ){
		...
		if($money>$amoney){
			$msg = "尊敬的{$uname}，您准备项目{$money}元，但您的账户可用余额为{$amoney}元，您要先去充值吗？";
			ajaxmsg($msg,2);
		}else{
			$msg = "尊敬的{$uname}，您的账户可用余额为{$amoney}元，您确认项目{$money}元吗？";
			ajaxmsg($msg,1);
		}
		ajaxmsg($msg,1);
	}
	...
	public function investmoney(){
		...
		$uname = session('u_user_name');
		if($amoney<$money) $this->error("尊敬的{$uname}，您准备项目{$money}元，但您的账户可用余额为{$amoney}元，请先去充值再项目.",__APP__."/member/charge#fragment-1");
		...
		if(($capital+$money)>$binfo['borrow_max']&&$binfo['borrow_max']>0){
			$xtee = $binfo['borrow_max'] - $capital;
			$this->error("您已项目{$capital}元，此投上限为{$binfo['borrow_max']}元，你最多只能再投{$xtee}");
		}
		//if($binfo['has_vouch']<$binfo['borrow_money'] && $binfo['borrow_type'] == 2) $this->error("此标担保还未完成，您可以担保此标或者等担保完成再项目");
		$need = $binfo['borrow_money'] - $binfo['has_borrow'];
		$caninvest = $need - $binfo['borrow_min'];
		if( $money>$caninvest && $need==0){
			$msg = "尊敬的{$uname}，此标已被抢投满了,下次项目手可一定要快呦！";
			$this->error($msg);
		}
		if(($binfo['borrow_min']-$money)>0 ){
			$this->error("尊敬的{$uname}，本标最低项目金额为{$binfo['borrow_min']}元，请重新输入项目金额");
		...
		if($done===true) {
			$this->success("恭喜成功项目{$money}元");
		}else if($done){
			$this->error($done);
		}else{
			$this->error("对不起，项目失败，请重试!");
		}
	}
	...
	public function ajax_invest(){
		...
		if($vo['borrow_status'] != 2) ajaxmsg("只能投正在项目中的标",0);
	...
	public function ajax_vouch(){
		...
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		////////////////////项目时自动填写可项目金额在项目文本框 2013-07-03 fan////////////////////////
		if($vo['vouch_progress']==100){
			$amoney = $vm['account_money']+$vm['back_money'];
			if($amoney<floatval($vo['borrow_min'])){
				ajaxmsg("您的账户可用余额小于本标的项目项目金额限制，项目项目！",0);

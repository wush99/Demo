...
class FriendAction extends MCommonAction {
	...
	public function addliuyan(){
		//echo "sssssssss";exit;
		if(empty($_POST['fasong'])){
          $res=M('members')->getField('id,id,user_name');
		  $str='<select name="touserid">';
		  foreach($res as $key=>$v){
             $str.='<option value="'.$v['id'].'">'.$v['user_name'].'</option>';
		  }
		  $str.='</select>';
		  $this->assign("touser",$str);
		  $this->display("addliuyan");
		   exit;
		}
		$touid =$_POST['touserid'];
		$msg = text($_POST['msg']);	
		$title = text($_POST['title']);	
		$newid = addMsg($this->uid,$touid,$title,$msg);
		if($newid){ 
			$this->success("发送成功","__URL__/addliuyan");
		}
		else {
		    $this->success("发送失败","__URL__/addliuyan");
		}

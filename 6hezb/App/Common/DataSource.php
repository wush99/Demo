...
/**
* 统计项目信息（项目总额、放款笔数、已还总额、待还总额）
*     
*/
function loan_total_info()
{
    $info = array();
    $info['ordinary_total'] = M( "borrow_info" )->where( "borrow_status in(6,7,8,9)" )->sum( "borrow_money" ); //普通标项目总额 
    ...
    $info['ordinary_total'] += $transfer_total_money;  //项目总额
    $info['has_also'] += $transfer_also_money; //已还款总额
    $info['arrears'] += $transfer_arrears_money;  //未还款总额
    $info['num_total'] += $transfer_num_total;  //项目笔数
    return $info;     
}  

/**
* 获取用户投资收益汇总
* （净赚利息、项目奖励、推广奖励、续投奖励、线下充值奖励、收入总和、代收利息）
* 
* @param int $uid  //用户ID
*/
function get_personal_benefit($uid)
{
    ...
    $benefit['ireward'] = $log['20']['money'] +  $log['41']['money'];     // 项目奖励
...
/**
*   用户项目支出汇总
* 、支付项目奖励、支付利息、提现手续费、项目管理费、会员及认证费用、逾期及催收费用 、 支出总和、待付利息总额
* 
...
{
    $log = get_money_log($uid);
    $out['borrow_manage'] = $log['18']['money']; //项目管理费
    $out['pay_tender'] = $log['21']['money']+$log['42']['money'];                   //支付项目奖励
...
function get_personal_count($uid)
{
    ...
    $borrow_fee = M('borrow_info')->where("borrow_uid={$uid} AND borrow_status in(6,7,8,9,10)")->sum('borrow_fee');  // 项目管理费 （统计复审通过后的管理费）
...
/**
* 项目参数\累计款金额\累计充值金额\累计提现金额\累计支付佣金
* 
* @param mixed $uid
*/
...
function autotInvest($borrow_id){
	...
	foreach($autolist as $key=>$v){
		...
		$num_max4 = $binfo['transfer_total']/10;//不能超过10%
		...
		if($v['duration_from'] > 0 && $v['duration_to'] > 0 && $v['duration_from'] <= $v['duration_to']){//项目期限范围
...
function autoInvest($borrow_id){
	...
	foreach($autolist as $key=>$v){
		...
		if($v['invest_money'] > $binfo['borrow_max'] && $binfo['borrow_max']>0){ // 大于最大项目 且设置最大项目
            $investMoney = $binfo['borrow_max'];
        }
		if($num_max1 > $v['invest_money']){//如果可用的投资金额大于最大投资金额，则投资金额等于最大投资金额
		    $investMoney = $v['invest_money'];
		}else{
		    $investMoney = $num_max1;//如果未设置项目后账户余额，则会投出全部余额
		}
		if($investMoney > $needMoney){
		    $investMoney = $needMoney;
		}else if($binfo['borrow_min']){ //设置了最小项目    如果直接满标则不考虑最小项目
            if($investMoney < $binfo['borrow_min']){ // 小于最低项目
                continue;//不符合最低投资金额
            } elseif(($needMoney-$investMoney)>0 && ($needMoney-$investMoney) < $binfo['borrow_min']){ // 剩余金额小于最小项目金额 
                if(($investMoney-$binfo['borrow_min']) >= $binfo['borrow_min']){  // 募集金额- 最小投资金额 大于最小投资金额
		...
		if($investMoney > $num_max4){//投资金额不能大于项目金额的10%

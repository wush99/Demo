...
class BorrowAction extends HCommonAction {
	...
	public function save(){
		...
		if($borrow['borrow_type']==0) $this->error("校验数据有误，请重新发布");
			// $this->error($_POST['borrow_type']);
		if(floatval($_POST['borrow_interest_rate'])>$rate_lixt[1] || floatval($_POST['borrow_interest_rate'])<$rate_lixt[0]) $this->error("提交的项目利率超出允许范围，请重试",0);
		...
		switch($borrow['borrow_type']){
			case 1://普通标
				if($_minfo['credit_cuse']<$borrow['borrow_money']) $this->error("您的可用信用额度为{$_minfo['credit_cuse']}元，小于您准备项目的金额，不能发标");
			break;
			case 2://新担保标
			case 3://秒还标
			break;
			case 4://净值标
				...
				// if($_netMoney<$borrow['borrow_money']) $this->error("您的净值额度{$_netMoney}元，小于您准备项目的金额，不能发标");
			break;
			case 5://抵押标
				//$borrow_type=5;
			break;
		...
		if($borrow['borrow_type']==3){//秒还标
			...
			if(($_minfo['account_money']+$_minfo['back_money'])<($borrow['borrow_fee']+$_reward_money)) $this->error("发布此标您最少需保证您的帐户余额大于等于".($borrow['borrow_fee']+$_reward_money)."元，以确保可以支付项目管理费和项目奖励费用");
		}
		...
		if($newid) $this->success("项目发布成功，网站会尽快初审",__APP__."/member/borrowin#fragment-2");

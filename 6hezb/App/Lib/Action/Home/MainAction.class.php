	...
	public function ajax_invest(){
		...
		if($binfo['money_collect']>0){
			if($vm['money_collect']<$binfo['money_collect']) {
				ajaxmsg("此标设置有项目待收金额限制，您账户里必须有足够的待收才能投此标",0);
			}
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		...
		if($amoney<floatval($vo['borrow_min'])){
			ajaxmsg("您的账户可用余额小于本标的最小项目金额限制，不能项目！",0);
...
//		$money = 100;
		...
		//if($binfo['has_vouch']<$binfo['borrow_money'] && $binfo['borrow_type'] == 2) ajaxmsg("此标担保还未完成，您可以担保此标或者等担保完成再项目",3);
		if(!empty($binfo['password'])){
			if(empty($borrow_pass)) ajaxmsg("此标是定向标，必须验证项目密码",3);
			else if($binfo['password']<>md5($borrow_pass)) ajaxmsg("项目密码不正确",3);
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		if($binfo['money_collect']>0){
			if($vm['money_collect']<$binfo['money_collect']) {
		...
		if(($capital+$money)>$binfo['borrow_max']&&$binfo['borrow_max']>0){
			...
			ajaxmsg("您已投资{$capital}元，此投上限为{$binfo['borrow_max']}元，你最多只能再投{$xtee}",3);
		}
		
		$need = $binfo['borrow_money'] - $binfo['has_borrow'];
		$caninvest = $need - $binfo['borrow_min'];
		if( $money>$caninvest && ($need-$money)<>0 ){
			$msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您投资{$money}元，将导致最后一次投资最多只能投".($need-$money)."元，小于最小投资金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>或者投资金额必须<font color='#FF0000'>小于等于{$caninvest}元</font>";
			if($caninvest<$binfo['borrow_min']) $msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您投资{$money}元，将导致最后一次投资最多只能投".($need-$money)."元，小于最小投资金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>即投资金额必须<font color='#FF0000'>等于{$need}元</font>";
		...
		if($money>$amoney){
			$msg = "尊敬的{$uname}，您准备投资{$money}元，但您的账户可用余额为{$amoney}元，请先去充值!";
			ajaxmsg($msg,2);
		}else{
			$msg = "尊敬的{$uname}，您的账户可用余额为{$amoney}元，您确认投资{$money}元吗？";
...
//		
		...
		$uname = session('u_user_name');
		if($amoney<$money) ajaxmsg("尊敬的{$uname}，您准备投资{$money}元，但您的账户可用余额为{$amoney}元，请先去充值再投资.",3);
		...
		if($binfo['money_collect']>0){
			if($m['money_collect']<$binfo['money_collect']) {
				ajaxmsg("此标设置有投资待收金额限制，您账户里必须有足够的待收才能投此标",3);
			}
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		...
		//if($binfo['has_vouch']<$binfo['borrow_money'] && $binfo['borrow_type'] == 2) $this->error("此标担保还未完成，您可以担保此标或者等担保完成再投资");
		$need = $binfo['borrow_money'] - $binfo['has_borrow'];
		$caninvest = $need - $binfo['borrow_min'];
		if( $money>$caninvest && ($need-$money)<>0 ){
			$msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您投资{$money}元，将导致最后一次投资最多只能投".($need-$money)."元，小于最小投资金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>或者投资金额必须<font color='#FF0000'>小于等于{$caninvest}元</font>";
		...
		if($done===true) {
			$_msg['type'] = $arr['type'];
			$_msg['id'] = $borrow_id;
			$_msg['message'] = "恭喜成功投资{$money}元";
			ajaxmsg($_msg,1);
			
		}else if($done){
			ajaxmsg($done,3);
		}else{
			ajaxmsg("对不起，投资失败，请重试!",3);
		}
	}

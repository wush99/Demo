...
class HelpAction extends HCommonAction {
    public function index(){
		...
		if ($vo['parent_id'] == 7) {
			$member['user_name'] = $vo['type_nid'];
			$borrow_uid = M('members')->getFieldByUserName($vo['type_nid'],'id');
			$borrowInfos = M('transfer_borrow_info')->where("borrow_uid={$borrow_uid} AND borrow_status=2")->select();
			$borrowInfosDone = M('transfer_borrow_info')->where("borrow_uid={$borrow_uid} AND borrow_status=7 AND transfer_out = transfer_total")->select();
			$allMoney = 0;
			foreach ($borrowInfos as $key => $value) {
					$allMoney += $value['borrow_money'];
			}
			foreach ($borrowInfosDone as $key => $value) {
					$allMoneyDone += $value['borrow_money'];
			}
			$this->assign('borrowInfos', $borrowInfos);
			$this->assign('borrowInfosDone', $borrowInfosDone);
			$this->assign('allMoney', $allMoney);
			$this->assign('allCount', count($borrowInfos));
			$this->assign('allMoneyDone', $allMoneyDone);
			$this->assign('allCountDone', count($borrowInfosDone));

			$typeinfo['templet'] = 'danbao';
		}

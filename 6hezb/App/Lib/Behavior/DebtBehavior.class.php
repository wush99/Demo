   ...
   class DebtBehavior
   {
       ...
       */
        ...
        */
        ...
        {  
            $count = M('borrow_investor')->where("investor_uid = ".$this->uid."  and status = 4 and debt_status=0")->count();
            $Page = new Page($count, $this->pagesize);
            $transfer = M('borrow_investor i')
                ->join("{$this->pre}borrow_info b ON b.id = i.borrow_id")
                ->join("{$this->pre}members m ON i.borrow_uid = m.id")
                ->field("i.id, i.borrow_id, i.add_time, i.deadline, i.investor_interest, i.investor_capital, b.borrow_name, b.borrow_interest_rate, m.user_name")
                ->where("i.investor_uid = ".$this->uid."  and i.status = 4 and i.debt_status=0")
        ...
        */
        public function countDebt($invest_id)
        {
           ...
           //可转让期数、统计待收本金和利息
           $debt = M("investor_detail")->field("count(id) as re_num, sum(capital) as capital, sum(interest) as interest ")->where($condition)->find();
           $debt['total'] = M("investor_detail")->where("invest_id=".$invest_id)->count('id'); //总共多少期
           return $debt;
        ...
        */
        ...
        {
            $condition = "invest_id= '".$invest_id."' and status in (6,7)";
            $total = M("investor_detail")->field("sum(capital+interest) as total_money ")->where($condition)->find();
        ...
        */
        ...
        {    
            $count = M('invest_detb')->where("sell_uid = ".$this->uid."  and status in (2,99)")->count();
            $Page = new Page($count, $this->pagesize);
            $Bonds['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        public function cancelDebt($invest_id, $type)
        {
            ...
            $condition2 =  " invest_id={$invest_id} and status=2";
            $this->Model->startTrans();
        ...
        {  
            $count = M('invest_detb')->where("sell_uid = ".$this->uid."  and status = 3")->count();
            $Page = new Page($count, $this->pagesize);
            $Bonds['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
        ...
        */
        ...
        {  
            $count = M('invest_detb')->where("sell_uid = ".$this->uid."  and status in (1,4)")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->join("{$this->pre}members m ON d.buy_uid=m.id")
                ->field("d.invest_id, i.borrow_id, d.money, d.transfer_price, d.buy_time, d.status, m.user_name,  
        ...
        */
        ...
        {
            $count = M('invest_detb')->where("buy_uid = ".$this->uid."  and status in (1,4)")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->join("{$this->pre}members m ON d.sell_uid=m.id")
        ...
        */
        ...
        {
            $count = M('invest_detb')->where("buy_uid = ".$this->uid."  and status = 1")->count();
            $Page = new Page($count, $this->pagesize);
            $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->join("{$this->pre}members m ON b.borrow_uid=m.id")
        ...
        */
        public function getAlso($invest_id)
        {
            $invest_id = intval($invest_id);
            $count = array();
            $count = M("investor_detail")->field("sum(interest) as interest, sum(capital) as capital")
                                ->where("invest_id={$invest_id}  and status in (1,2,3,4,5)")
                                ->find();
            $info = M("investor_detail")->field("deadline, sort_order, expired_days")
        ...
        */
        public function listAll($parm,$size=0, $pstatus=true)
        {
            ...
            if($pstatus){ 
                //$count = M("invest_detb")->where("status in (2, 4)")->count();
				$count = M("invest_detb d")->where($map)->count();
                $Page = new Page($count, $this->pagesize); 
                $list['data'] = M("invest_detb d")
                        ->join("{$this->pre}borrow_investor i ON d.invest_id=i.id")
                        ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                        ->join("{$this->pre}members m ON i.investor_uid=m.id")
                        ->field($field)
                        ->where($map)
                        ->limit($Page->firstRow .','. $Page->listRows)
                        ->order("d.status asc")
        ...
        */
        private function checkBuy($paypass, $invest_id)
        {
            ...
            $user_info = M("members m")
                            ->join("{$this->pre}member_money money ON m.id = money.uid")
